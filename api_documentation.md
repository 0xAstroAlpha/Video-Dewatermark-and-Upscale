...
*   **Base URL**: `http://127.0.0.1:8288` (hoặc `http://localhost:8288`)
*   **Giao thức**: HTTP/1.1
...
```bash
curl -X POST "http://127.0.0.1:8288/process" \
...
url = "http://127.0.0.1:8288/process"
payload = {
...
fetch('http://127.0.0.1:8288/process', {
    method: 'POST',
    body: formData
})
.then(response => {
